[![Build and test the esy package](https://github.com/ManasJayanth/esy-gn/actions/workflows/workflow.yml/badge.svg)](https://github.com/ManasJayanth/esy-gn/actions/workflows/workflow.yml)

# esy-gn

`esy-gn` is sample package, [`gn`](https://gn.googlesource.com/gn), packaged for [`esy`](https://esy.sh/).
...
`esy-gn` can be used from both NPM and directly from Github.

### From NPM 

`esy-gn` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-gn` to install it, or specify it in
...
```diff
  "dependencies": {
+   "esy-gn": "esy-packages/esy-gn#<commit hash>"
  }
```
